...
This site uses a file-based routing system, based on the file structure in the `data/` directory. Page slugs are automatically generated and assigned to each document, based on its file name and location in the file structure.
...
To assign a page to a category, set its `category` field to the **ID of the desired category.** If a page does not have a `category` field, or its category value isn't one of the category IDs defined in `data/config.json`, **it is considered uncategorized** and displayed at the top of the sidebar.
